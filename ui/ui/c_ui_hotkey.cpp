...
void c_ui_hotkey::draw(c_ui_window *wnd) {
    ...
    d->AddRectFilled(control_rect.Min, control_rect.Max, ui->active_id == id ? ui->style.accent : ui->style.bottom);
    ...
    if (val) {
        if (!beh)
            d->AddText(control_rect.Min + ImVec2(5.f, 4.f), ui->style.text_normal, keys.at(*val).c_str());
        else {
            auto txt = keys.at(*val);
            if (*beh == 0) txt += " (always on)";
            if (*beh == 1) txt += " (hold)";
            if (*beh == 2) txt += " (toggle)";
            if (*beh == 3) txt += " (release)";

            d->AddText(control_rect.Min + ImVec2(5.f, 4.f), ui->style.text_normal, txt.c_str());
        }
    }
    else
        d->AddText(control_rect.Min + ImVec2(5.f, 4.f), ui->style.text_disabled, "none");

    ui->set_current_id(beh_id);
    if (ui->active_id == beh_id && beh && !clear) {
...
bool c_ui_hotkey::get_state() {
    return keymanager->check(get_value(), get_behaviour());
}
