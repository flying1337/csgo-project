...
void c_ui_colorpicker::draw(c_ui_window *wnd) {
            ...
            if (ui->is_hovered(rect))
            {
                if (dd == "Copy") {
                    ...
                    sprintf(buffer, "%02X%02X%02X%02X",
                            (int)(val[0] * 255.f), (int)(val[1] * 255.f),
                            (int)(val[2] * 255.f), (int)(val[3] * 255.f));

                    ImGui::SetClipboardText(buffer);

                    ui->add_message("Copied", "Color has been copied into clipboard!");
                }

                if (dd == "Paste") {
                    int r = 0, g = 0, b = 0, a = 0;
                    sscanf_s(ImGui::GetClipboardText(), "%02X%02X%02X%02X", &r, &g, &b, &a);
    ...
    d->AddRectFilled(control_rect.Min, control_rect.Max, val ? ImColor(val[0], val[1], val[2]) : ui->style.bottom);
    ...
    ui->set_current_id(beh_id);
    if (ui->active_id == beh_id && val && !clear) {
    ...
    if (ui->active_id == id && val && !clear) {
        ...
        f->AddRect(picker_rect.Min, picker_rect.Max, ui->style.outline);
        f->AddRect(picker_rect.Min + ImVec2(1.f, 1.f), picker_rect.Max - ImVec2(1.f, 1.f), ui->style.shadow);
        ...
        f->AddRect(hue_rect.Min, hue_rect.Max, ui->style.outline);
        f->AddRect(hue_rect.Min + ImVec2(1.f, 1.f), hue_rect.Max - ImVec2(1.f, 1.f), ui->style.shadow);
        ...
        if (draw_alpha_bar) {
            ...
            f->AddRect(alpha_rect.Min, alpha_rect.Max, ui->style.outline);
            f->AddRect(alpha_rect.Min + ImVec2(1.f, 1.f), alpha_rect.Max - ImVec2(1.f, 1.f), ui->style.shadow);
