...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	window->DrawList->AddRectFilled(bb.Min, bb.Max, held ? menu->get_accent_color(1.f) : menu->get_dark_color());
	window->DrawList->AddRect(bb.Min, bb.Max, hovered ? menu->get_accent_color(1.f) : menu->get_outline_color());
	window->DrawList->AddRect(bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), menu->get_shadow_color());
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
	...
	// Render
	window->DrawList->AddRectFilled(bb_frame.Min, bb_frame.Max, menu->get_outline_color(), window->WindowRounding, rounding_corners);
	const ImU32 grab_col = menu->get_light_color();
	ImRect grab_rect;
	if (horizontal)
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	window->DrawList->AddRectFilled(pos + ImVec2(square_sz, square_sz) * 0.25f, pos + ImVec2(square_sz, square_sz) * 0.75f, *v ? menu->get_accent_color(1.f) : menu->get_dark_color());
	window->DrawList->AddRect(pos + ImVec2(square_sz, square_sz) * 0.25f + ImVec2(1, 1), pos + ImVec2(square_sz, square_sz) * 0.75f - ImVec2(1, 1), menu->get_shadow_color());

	window->DrawList->AddRect(pos + ImVec2(square_sz, square_sz) * 0.25f, pos + ImVec2(square_sz, square_sz) * 0.75f, IsItemHovered() ? menu->get_accent_color(1.f) : menu->get_outline_color());
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 2.f), frame_bb.Max, menu->get_dark_color());
	window->DrawList->AddRect(frame_bb.Min + ImVec2(0, 2.f), frame_bb.Max, hovered ? menu->get_accent_color(1.f) : menu->get_outline_color());
	window->DrawList->AddRect(frame_bb.Min + ImVec2(1, 3.f), frame_bb.Max - ImVec2(1, 1), menu->get_shadow_color());
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
	...
	window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 6.f), frame_bb.Max, menu->get_dark_color());

	if (data_type == ImGuiDataType_S32) {
		float r_val = (float) * (int*)v, r_min = (float) * (int*)v_min, r_max = (float) * (int*)v_max;

		auto max_w = frame_bb.Max.x - frame_bb.Min.x;
		auto max_h = frame_bb.Max.y - frame_bb.Min.y;

		/*if (r_val > r_max * 0.5f)
			r_val += 0.5f;*/

		window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 6.f), frame_bb.Min + ImVec2(max_w / (r_max - r_min) * (r_val - r_min), max_h), menu->get_accent_color(1.f));
	}

	if (data_type == ImGuiDataType_Float) {
		float r_val = *(float*)v, r_min = *(float*)v_min, r_max = *(float*)v_max;
	...
	window->DrawList->AddRect(frame_bb.Min + ImVec2(0, 6.f), frame_bb.Max, (IsItemHovered() || IsItemActive()) ? menu->get_accent_color(1.f) : menu->get_outline_color());
	window->DrawList->AddRect(frame_bb.Min + ImVec2(1, 7.f), frame_bb.Max - ImVec2(1, 1), menu->get_shadow_color());
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	if (!is_multiline)
	{
		window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, menu->get_dark_color());
		window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, hovered ? menu->get_accent_color(1.f) : menu->get_outline_color());
		window->DrawList->AddRect(frame_bb.Min + ImVec2(1, 1), frame_bb.Max - ImVec2(1, 1), menu->get_shadow_color());
	}
	...
	if (render_cursor || render_selection)
	{
		...
		if (render_cursor)
		{
			...
			if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
				draw_window->DrawList->AddLine(cursor_screen_rect.Min, cursor_screen_rect.GetBL(), menu->get_accent_color(1.f));
...
bool ImGui::ColorButton(const char* desc_id, ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
	...
	window->DrawList->AddRect(bb.Min, bb.Max, hovered ? menu->get_accent_color(1.f) : menu->get_outline_color(), rounding); // Color button are often in need of some sort of border
	window->DrawList->AddRect(bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), menu->get_shadow_color(), rounding);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	window->DrawList->AddText(bb.Min + ImVec2(8.f, 0), selected ? menu->get_accent_color(1.f) : (hovered ? ImColor(255, 255, 255) : ImColor(204, 204, 204)), label, strstr(label, "##"));
...
bool ImGui::TabButton(const char* text, int id, int max, int selected) {
	...
	if (selected == id)
		window->DrawList->AddRectFilled(bb.Min, bb.Max, menu->get_light_color());
	else {
	    window->DrawList->AddRectFilled(bb.Min, bb.Max, menu->get_dark_color());
		window->DrawList->AddRectFilledMultiColor(ImVec2(bb.Min.x, bb.Max.y - 6.f), bb.Max,
		        menu->get_shadow_color(0.f), menu->get_shadow_color(0.f), menu->get_shadow_color(), menu->get_shadow_color());
	}
...
bool ImGui::Hotkey(const char* text, int* key, int* behaviour) {
	...
	if (pressed && !is_active)
		SetActiveID(id, window);
	else {
		if (is_active) {
			if (keymanager->is_key_down(VK_ESCAPE)) {
				*key = 0;
				SetActiveID(0, window);
			}
			else {
				if (keymanager->is_key_down(VK_LBUTTON))
				{
					*key = VK_LBUTTON;
					SetActiveID(0, window);
					goto DRAW;
				...
				if (keymanager->is_key_down(VK_RBUTTON))
				{
					*key = VK_RBUTTON;
					SetActiveID(0, window);
					goto DRAW;
				...
				if (keymanager->is_key_down(VK_MBUTTON))
				{
					ctx->IO.MouseDown[2] = false;
					*key = VK_MBUTTON;
					SetActiveID(0, window);
					goto DRAW;
				}

				if (keymanager->is_key_down(VK_XBUTTON1))
				{
					*key = VK_XBUTTON1;
					SetActiveID(0, window);
					goto DRAW;
				...
				if (keymanager->is_key_down(VK_XBUTTON2))
				{
					*key = VK_XBUTTON2;
					SetActiveID(0, window);
					goto DRAW;
			...
			if (behaviour) {
				if (!pressed && hovered && ctx->IO.MouseClicked[1]) {
					OpenPopupEx(id);
					popup_open = true;
				}

				if (popup_open) {
					SetNextWindowSizeConstraints(ImVec2(165.f, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(-1)));
					...
					if (Selectable("Always On", *behaviour == 0))
						* behaviour = 0;
					if (Selectable("Hold", *behaviour == 1))
						* behaviour = 1;
					if (Selectable("Toggle", *behaviour == 2))
						* behaviour = 2;
					if (Selectable("Inverse", *behaviour == 3))
						* behaviour = 3;
...
DRAW:
	...
	window->DrawList->AddRectFilled(bb.Min, bb.Max, is_active ? menu->get_accent_color(1.f) : menu->get_dark_color());
	window->DrawList->AddRect(bb.Min, bb.Max, hovered ? menu->get_accent_color(1.f) : menu->get_outline_color());
	window->DrawList->AddRect(bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), menu->get_shadow_color());
