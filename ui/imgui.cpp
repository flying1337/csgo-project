...
void ImGui::EndChild()
{
	...
	if (is_correct_group)
	{
		window->DrawList->AddRect(window->Pos, window->Pos + window->Size, menu->get_outline_color());

		window->DrawList->AddRectFilledMultiColor(window->Pos + ImVec2(1, window->TitleBarHeight()), window->Pos + ImVec2(window->Size.x - 9.f, window->TitleBarHeight() + 10.f),
			menu->get_light_color(), menu->get_light_color(), menu->get_light_color(0.f), menu->get_light_color(0.f));
		window->DrawList->AddRectFilledMultiColor(window->Pos + ImVec2(1, window->Size.y - 1), window->Pos + ImVec2(window->Size.x - 9.f, window->Size.y - 11.f),
			menu->get_light_color(), menu->get_light_color(), menu->get_light_color(0.f), menu->get_light_color(0.f));
	}
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
	...
	if (!(flags & ImGuiWindowFlags_NoTitleBar)) {
		window->DrawList->AddRectFilled(title_bar_rect.Min, title_bar_rect.Max, is_correct_group ? menu->get_outline_color() : menu->get_dark_color());

		if (flags & ImGuiWindowFlags_MessageBox)
			window->DrawList->AddRectFilledMultiColor(title_bar_rect.Min + ImVec2(0, title_bar_rect.Max.y - title_bar_rect.Min.y - 6.f), title_bar_rect.Max,
	...
	if (flags & ImGuiWindowFlags_Popup || flags & ImGuiWindowFlags_Tooltip || flags & ImGuiWindowFlags_Listbox) {
		window->DrawList->AddRectFilledMultiColor(window->Pos, window->Pos + ImVec2(window->Size.x, 6.f),
			menu->get_shadow_color(), menu->get_shadow_color(), menu->get_shadow_color(0.f), menu->get_shadow_color(0.f));
		window->DrawList->AddRectFilledMultiColor(window->Pos, window->Pos + ImVec2(6.f, window->Size.y),
			menu->get_shadow_color(), menu->get_shadow_color(0.f), menu->get_shadow_color(0.f), menu->get_shadow_color());
		window->DrawList->AddRectFilledMultiColor(window->Pos + ImVec2(0, window->Size.y - 6.f), window->Pos + window->Size,
			menu->get_shadow_color(0.f), menu->get_shadow_color(0.f), menu->get_shadow_color(), menu->get_shadow_color());
		window->DrawList->AddRectFilledMultiColor(window->Pos + ImVec2(window->Size.x - 6.f, 0), window->Pos + window->Size,
			menu->get_shadow_color(0.f), menu->get_shadow_color(), menu->get_shadow_color(), menu->get_shadow_color(0.f));
	}
